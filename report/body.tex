...
mismatch with probability $\etaB$ (\etaloB--\etahiB, $\nB$ residues), against the
equilibrium bound $\eta_{\rm eq}=\etaeq$ of Eq.~\eqref{eq:etaeq}; the $\sim\!10\%$
excess is the deliberate compromise of \S2.3, the finite ratio
$k^{\rm com}/k^{\rm rej}$ (the self-consistent theory for this rate set gives
$\etaBtheory$).  Either way the selection step alone cannot do appreciably better than
the binding free energy allows.  Adding the discard arc and
...
Figure~\ref{fig:square} sweeps $\delta$ from $1.05$ to $2.45\,\kB T$ --- a factor of
four in the discrimination ratio $e^{\delta}$ --- and fits
$\mathrm{d}\ln\eta/\mathrm{d}\delta$.  Protocol~B gives $\slopeB$; protocol~C gives
