...
\end{center}
which pins $|\ka{M}|_{\rm free}$ to within a few parts in $10^3$ of its set point
(monitored, and reported with every run).  The \ka{[max](0)(...)} is not cosmetic:
...
\resizebox{\textwidth}{!}{\input{scheme}}
\vspace{0.6em}
...
(\etaloC--\etahiC, $\nC$ residues), a factor of $\gain$, and brings it to within about a
factor of two of the Hopfield floor $e^{-2\delta}=\etahop$.  The middle case is the
...
for the three machines; the slope is the error fraction.  (b) the running error
...
enough that its reverse is negligible, and it costs both speed
(panel~b: the growth velocity falls by $\slowdown\times$ between $\mu_{02}=0$ and the
plateau) and monomers ($\costC$ excision events per residue retained).
...
still grows at $41\%$ of the protocol-B velocity and the error is $\prefac\times$ the
...
self-consistent theory, together with the stack SSA.  The three implementations agree
across two decades in $\eta$, which is the evidence that the Kappa encoding --- in
particular the decision to represent active-site occupancy as a bond plus an internal
state, and the derived ligation rate --- is doing what it is supposed to.  The agreement
...
the polymerisation drive is reduced towards stall; the error converges on the
equilibrium bound.}
\label{fig:valid}
\end{figure}
...
uv run python kpr/experiments.py           # all 29 runs, 4-way parallel, ~2 h.
...
\noindent
\begin{tabular}{@{}ll@{}}
\ka{kpr/model.py} & the model: \ka{Params}, \ka{kappa()} (emits the \ka{.ka} source),
                    \ka{landscape()}\\
\ka{kpr/run.py} & one trajectory: seeding, sampling, summary statistics\\
\ka{kpr/experiments.py} & the job list (conditions and sweeps)\\
